...
const ChatList: React.FC<IChatListProps> = ({
  ...
  useEffect(() => {
    ...
    const unsubChannel = ChannelRepository.getChannel(
      chatId,
      ({ data: channel }) => {
        if (channel) {
          const { messagePreview: channelMessagePreview } = channel;

          if (channelMessagePreview && channelMessagePreview.data) {
            setPreviewMessage(
              channelMessagePreview.data.text ||
                channelMessagePreview.dataType[0].toUpperCase() +
                  channelMessagePreview.dataType.slice(1)
            );
          }
        }
      }
        ...
        <View style={styles.chatDetailSection}>
          ...
          {previewMessage && (
            <View style={styles.chatPreviewWrap}>
              <CustomText style={styles.chatPreviewMessage} numberOfLines={2}>
                {previewMessage}
              </CustomText>
              {unReadMessage > 0 && (
