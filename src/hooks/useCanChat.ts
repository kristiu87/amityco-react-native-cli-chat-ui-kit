...
export default function useCanChat(channelId: string) {
  ...
  useEffect(() => {
    const unsubscribe = ChannelRepository.Membership.getMembers(
      ...
      async ({ loading, data, hasNextPage, onNextPage }) => {
        ...
        const { data: users } = await UserRepository.getUserByIds(
          data.map((member) => member.userId)
        );

        console.log(users);

        const hasCS = users.some((user) =>
          user.roles.includes(roleConfig.customerServiceRoleId)
        );
